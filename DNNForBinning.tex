...
\begin{table}[htbp]
\centering
    \caption{k=4和5时，自动编码机在500k序列的模拟数据上的聚类结果。加粗数值表示某个数据集上的最好结果。}
...
通过查看k=4和k=5时，500k的5个数据集S6-S10在自动编码机上的性能比较，可以看出在S6、S8和S10上k=5时F1值更高。
S7和S9上k=4时F1值更高。

\subsection{自动编码机个数及层数的影响}
可以采用多个自动编码机来进行特征的多次提取，这样可以学习到更好的特征。
具体的，采用多层自动编码机对特征进行学习。

\begin{table}[htbp]
\centering
    \caption{自动编码机不同层数的聚类效果。}
    \begin{tabular}{|c|c|c|c|}
    \hline
    \multirow{2}{*}{\textbf{神经网络层数}} & \multicolumn{3}{|c|}{\textbf{AutoEncoder}}\\
    \cline{2-4}
...
\begin{table}[htbp]
\centering
    \caption{采用自动编码机和RBM在12个数据集上的聚类效果。}
    \begin{tabular}{|c|c|c|c|}
    \hline
  ...
  %  D1&\textbf{.9989}&.9628	&.9805&	.9877&	.9877	&.9877	&.9882	&\textbf{.9882}	&\textbf{.9882}\\
    ...
    \label{tab:9}
\end{table}
通过实验结果可以知道，自动编码机聚类效果在12个数据集上都优于RBM。
