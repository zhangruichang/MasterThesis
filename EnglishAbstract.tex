...
Currently, methods can be categoried into two groups: similarity-based methods and component-based methods. For similarity-based methods,
referece genome is needed, and they align reads to referece genome, and then bin reads according to the alignment results. However, for real metagenomic datasets, most species are unknown, so these methods are not available. Component-based methods first directly analyze the sequences and extract features to represent sequences. As reference genomes are not needed, unsupervised learning are more popular recently, researchers proposed AbundanceBin, MetaCluster and MCluster one after another.
...
effectively improve the binning performance of metagenomic reads. Our experiments show that deep learning also improve the performance of 
binning.
