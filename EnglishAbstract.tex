...
Currently, methods of metagenomic data binning can be categoried into two groups: similarity-based methods and component-based methods. For similarity-based methods,
referece genome is needed, and they align reads to referece genome, and then bin reads according to the alignment results. However, for real metagenomic datasets, most species are unknown, so these methods are not applicable. Component-based methods first directly analyze the sequences and extract features to represent sequences. As reference genomes are not needed, unsupervised learning based methods are popular recently, a number of such methods including AbundanceBin, MetaCluster, MCluster, etc. are developed.
...
Furthermore, we also explore deep learning for metagenomic sequences binning task. Firstly, we exploit AutoEncoder and RBM to learn useful features based on $k$-mer features, then utilize SKWIC algorithm to cluster these reads. This method also achieves satisfactory performance.
...
effectively improve the binning performance of metagenomic reads. Our experiments show that deep learning also can improve the performance of binning.


\bigskip
\bigskip

\noindent{\bf Keywords}: Metagenomics, Metagenomic data binning, Topic modeling, Deep learning
