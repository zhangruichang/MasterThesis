...
相对而言高丰度的序列(50k和500k序列)，因为AbundanceBin是专门处理高丰度的序列。10个数据集分别称为S1到S10。这些数据集的细节列在表
...
\section{本章小结}
在本节中，我们在模拟数据和真实数据集上，对我们的方法进行了全方面的评估，并且与已有的MetaCluster 3.0 5.0 AbundanceBin 以及MCluster进行了比较分析，结果显示我们的方法在多数数据集上聚类性能都优于上述方法。
